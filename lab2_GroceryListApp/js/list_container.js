...
if (Items.length > 0) {
	Items.forEach((Item) => {
		list_Items.innerHTML =
			list_Items.innerHTML +
			`<div class="list_item flex items-center gap-3 rounded-xl bg-white p-3 border border-gray-200 shadow-sm hover:shadow-md transition">
							<input type="checkbox" class="form-checkbox h-4 w-4 text-blue-500 rounded flex-shrink-0" ${Item.completed ? "checked" : ""}/>
							<input type="text" class="${Item.completed ? "line-through" : ""} bg-gray-100 text-gray-900 text-lg px-2 py-1 rounded-lg flex-1 min-w-0 focus:outline-none" readonly value="${Item.value}" name="${Item.value}"/>
		...
		if (e.target.checked) {
			...
			list_Items.innerHTML =
				...
				`<div class="list_item flex items-center gap-3 rounded-xl bg-white p-3 border border-gray-200 shadow-sm hover:shadow-md transition">
								...
								<input type="text" class="line-through bg-gray-100 text-gray-900 text-lg px-2 py-1 rounded-lg flex-1 min-w-0 focus:outline-none" readonly value="${e.target.nextElementSibling.value}" name="${e.target.nextElementSibling.value}"/>
								<button class="btn_edit bg-gray-100 hover:bg-gray-200 text-gray-900 px-3 py-1.5 rounded-lg text-sm shadow transition flex-shrink-0">Edit</button>
					<button class="btn_delete bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-sm shadow transition flex-shrink-0" aria-label="Delete">x</button>
							</div>`;
			...
			list_Items.innerHTML =
				`<div class="list_item flex items-center gap-3 rounded-xl bg-white p-3 border border-gray-200 shadow-sm hover:shadow-md transition">
							...
							<input type="text" class="bg-gray-100 text-gray-900 text-lg px-2 py-1 rounded-lg flex-1 min-w-0 focus:outline-none" readonly value="${e.target.nextElementSibling.value}" name="${e.target.nextElementSibling.value}"/>
							<button class="btn_edit bg-gray-100 hover:bg-gray-200 text-gray-900 px-3 py-1.5 rounded-lg text-sm shadow transition flex-shrink-0">Edit</button>
					<button class="btn_delete bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-sm shadow transition flex-shrink-0" aria-label="Delete">x</button>
